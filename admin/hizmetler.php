...
<?php
									$list = $db->query("SELECT * FROM ".TABLE." WHERE dil_id = '".LANGUAGE_DEFAULT."' ORDER BY row ASC LIMIT $baslangic,$limit");
										if ($list->rowCount()){
											foreach($list as $row){
?>
										<tr id="item-<?php echo $row["hizmet_ust_id"]; ?>" role="row" class="even">
											...
											<td class="sorting_1">
												<div class="d-flex">
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=edit&id=<?php echo $row["hizmet_ust_id"]; ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>
													<a class="contact-icon" href="<?php echo AREA; ?>?do=delete&id=<?php echo $row["hizmet_ust_id"]; ?>"><i class="fas fa-trash-alt"></i></a>
												</div>
											</td>
										</tr>
<?php
					...
					if(!empty($_POST['haber_baslik']) || !empty($_POST['haber_aciklama'])) {
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET haber_baslik = ?, haber_seo = ?, haber_aciklama = ?, haber_description = ?, haber_kisaaciklama = ?,  haber_resim = ?, dil_id = ?, haber_durum = ?, hizmet_ust_id = ?");
...
<?php
				...
				$row_check = $db->prepare("SELECT * FROM ".TABLE." WHERE hizmet_ust_id = ?");
				$row_check->execute(array($id));
				if ($row_check->rowCount() > 0) {
					if(isset($submitControl)){
						if(!empty($_POST['haber_baslik']) || !empty($_POST['haber_aciklama'])) {
							$image_info = $db->query("SELECT * FROM ".TABLE." WHERE hizmet_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
								$upload = new Upload($_FILES['haber_resim']);
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									...
									$update = $db->prepare("UPDATE ".TABLE." SET  haber_baslik = ?, haber_seo = ?, haber_aciklama = ?, haber_description = ?, haber_kisaaciklama = ?,  haber_resim = ?, haber_durum = ? WHERE hizmet_ust_id = ? AND dil_id = ?");
					...
					$row_info = $db->query("SELECT * FROM ".TABLE." WHERE hizmet_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
?>
												...
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Başlık</label>
																<input type="text" name="haber_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Başlık" required="" value="<?php echo GetTableValue("haber_baslik",TABLE,"where hizmet_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Description</label>
																<input type="text" name="haber_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Description" required="" value="<?php echo GetTableValue("haber_description",TABLE,"where hizmet_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Kısa Açıklaması</label>
																<input type="text" name="haber_kisaaciklama[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Kısa Açıklaması" required="" value="<?php echo GetTableValue("haber_kisaaciklama",TABLE,"where hizmet_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Açıklama</label>
																<textarea class="form-control ckeditor" name="haber_aciklama[<?php echo $LANGUAGE_CODE; ?>]"><?php echo GetTableValue("haber_aciklama",TABLE,"where hizmet_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?></textarea>
															</div>
														</div>
													</div>
													<input type="hidden" name="form_<?php echo $LANGUAGE_CODE; ?>" value="<?php echo $LANGUAGE_CODE; ?>" />
		...
		$( "#sortable" ).sortable({
			...
			stop: function (event, ui) {
				...
				$.ajax({
					...
					dataType: "json",
					data: data,
					url: "/include/ajax.php?do=service&action=row",
